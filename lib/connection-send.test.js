...
test('.send() should reject if connection is not open', async t => {
	...
	const m = await new M();
	await m.close();
	return t.throws(m.send(), 'instance not in state OPEN');
});

test('.send() should reject if not used with string', async t => {
	const M = mock();
	const m = await new M();
	await t.throws(m.send(), 'first argument must be a string');
	await t.throws(m.send(12), 'first argument must be a string');
	await t.throws(m.send({text: 'this'}), 'first argument must be a string');
});
